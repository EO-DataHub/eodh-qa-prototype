    ...
    with session_scope() as s:
            ...
            if sensor1 in "s2":
                for coll_id in coll_idx:
                    mups_list[idx] = (
                        mups_list[idx]
                        + s.query(Matchup)
                        .where(Matchup.sensor_1_collection_id == coll_id)
                        .where(Matchup.sensor_2_reference_id == ref_idx)
                        .where(Matchup.product_date.between(dates[0], dates[-1]))
                        .all()
                    )
            else:
                mups_list[idx] = (
                    mups_list[idx]
                    + s.query(Matchup)
                    .where(Matchup.sensor_1_collection_id == coll_idx)
                    .where(Matchup.sensor_2_reference_id == ref_idx)
                    .where(Matchup.product_date.between(dates[0], dates[-1]))
                    .all()
                )
        ...
        for idx, ref in enumerate(sensor2):
            ...
            for k, mup in enumerate(mups_list[idx]):
                mup_idx = mup.id
                bias_vals_db = (
                    s.query(BiasVals).where(BiasVals.matchup_id == mup_idx).all()
                )
                bias_unc_vals_db = (
                    s.query(BiasUncVals).where(BiasUncVals.matchup_id == mup_idx).all()
                )
                meas_vals_dbs = (
                    s.query(MeasVals).where(MeasVals.matchup_id == mup_idx).all()
                )
                ...
                mup.sensor_1_wavelengths = [
                    val for val in mup.sensor_1_wavelengths if val != "9999.0"
                ]

                # add in attrs and data_vars per matchup
                mup_info.attrs[sensor1 + "_" + ref + "_" + "Datetime"][
                    k
                ] = mup.product_date
                mup_info[str(sensor1) + "_" + str(ref) + "_BiasVals"].values[k] = (
                    xr.DataArray(data=bias_vals.flatten())
                )
                mup_info[str(sensor1) + "_" + str(ref) + "_BiasUncVals"].values[k] = (
                    xr.DataArray(data=bias_unc_vals.flatten())
                )
                mup_info[str(sensor1) + "_" + str(ref) + "_MeasValsSensor1"].values[
                    k
                ] = xr.DataArray(data=meas_vals_sensor1.flatten())
                mup_info[str(sensor1) + "_" + str(ref) + "_MeasValsSensor2"].values[
                    k
                ] = xr.DataArray(data=meas_vals_sensor2.flatten())
                mup_info.attrs[str(sensor1) + "_" + str(ref) + "_Metadata"][
                    str(mup.id)
                ] = {
                    "Mission & Reference Site": sat_name[sensor1]
                    + " & "
                    + str(sensor2),
...
def plot_timeseries(
    ...
    for i in range(len(bands)):
        timeseries_fig.add_trace(
            go.Scatter(
                ...
                legendgroup="group1",
                legendgrouptitle_text=f"{refs[0]}",
            )
        )
        if len(refs) > 1:
            timeseries_fig.add_trace(
                go.Scatter(
                    ...
                    legendgroup="group2",
                    legendgrouptitle_text=f"{refs[-1]}",
                )
            )
    ...
    timeseries_fig.layout.margin = {"t": 10, "b": 10, "r": 0, "l": 20}
    ...
    for ref in refs:
        ...
        for k in range(len(band)):
            if sat == "planet":
                ...
                bias_vals[
                    ref + " B" + str(l8_bands_idx[band[k]] + 1) + " - " + str(band[k])
                ] = f"{rcn_bias_vals_mean[k]}%"
                ...
                if s2_bands_idx[band[k]] <= 7:
                    ...
                    bias_vals[ref + " B8A" + " - " + str(band[k])] = (
                        f"{rcn_bias_vals_mean[k]}%"
                    )
                else:
                    bias_vals[
                        ref + " B" + str(s2_bands_idx[band[k]]) + " - " + str(band[k])
                    ] = f"{rcn_bias_vals_mean[k]}%"
...
def analysis_table_update(sat, refs, mup_info):
    ...
    matchup_analysis_df["column_2"][0] = (
        sat_name[sat]
        + " & "
        + ref_name[refs[0]].split("-")[0][:-1]
        + ref_name[refs[0]].split("-")[1]
    )
